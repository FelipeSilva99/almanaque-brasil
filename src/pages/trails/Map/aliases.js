// Assets
import aguaMarinhaMap from '../../../images/trails/map/agua-marinha.svg'
import alexandritaMap from '../../../images/trails/map/alexandrita.svg'
import ametistaMap from '../../../images/trails/map/ametista.svg'
import diamanteMap from '../../../images/trails/map/diamante.svg'
import esmeraldaMap from '../../../images/trails/map/esmeralda.svg'
import ouroMap from '../../../images/trails/map/ouro.svg'
import prataMap from '../../../images/trails/map/prata.svg'
import safiraMap from '../../../images/trails/map/safira.svg'
import topazioMap from '../../../images/trails/map/topazio.svg'
import turmalinaMap from '../../../images/trails/map/turmalina.svg'



const trailsMap = {
  "Água-Marinha": {
    "position": {bottom: "428px", left: "104px"},
    "img": aguaMarinhaMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Diamante": {
    "position": {bottom: "263px", left: "-6px"},
    "img": diamanteMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Ametista": {
    "position": {bottom: "0px", left: "-9px"},
    "img": ametistaMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Turmalina": {
    "position": {bottom: "319px", left: "223px"},
    "img": turmalinaMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Prata": {
    "position": {bottom: "102px", left: "-6px"},
    "img": prataMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Ouro": {
    "position": {bottom: "118px", left: "142px"},
    "img": ouroMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Esmeralda": {
    "position": {bottom: "262px", left: "166px"},
    "img": esmeraldaMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Safira": {
    "position": {bottom: "98px", left: "287px"},
    "img": safiraMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Alexandrita": {
    "position": {bottom: "0", left: "141px"},
    "img": alexandritaMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  },

  "Topázio": {
    "position": {bottom: "384px", left: "-9px"},
    "img": topazioMap,
    "stone": {
      "todo": "",
      "doing": "",
      "done": ""
    }
  }

}

export default trailsMap;